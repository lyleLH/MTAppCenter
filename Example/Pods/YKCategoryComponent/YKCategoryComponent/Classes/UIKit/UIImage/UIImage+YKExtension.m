...
+ (UIImage *)yk_randomColorImage {
    return [self yk_fixedSizeRandomColorImageWithSize:CGSizeMake(44, 44)];
}

/// 用随机颜色生成一个指定size的Image对象
/// @param size 图片的尺寸
+ (UIImage *)yk_fixedSizeRandomColorImageWithSize:(CGSize)size {
    return [self yk_colorImageWithColor:[UIColor yk_randomColor] andSize:size];
}
...
+ (NSData *)yk_compressImageWithImage:(UIImage *)image aimWidth:(CGFloat)width aimLength:(NSInteger)length accuracyOfLength:(NSInteger)accuracy {
    UIImage * newImage = [self yk_scaleImage:image toSize:CGSizeMake(width, width * image.size.height / image.size.width)];
...
+ (UIImage *)yk_fitScreenWithImage:(UIImage *)image
{
    ...
    if (min && image.size.width<screenWidth) {
     ...
     image = [self yk_scaleImage:image toSize:newSize];
    return image;
}
