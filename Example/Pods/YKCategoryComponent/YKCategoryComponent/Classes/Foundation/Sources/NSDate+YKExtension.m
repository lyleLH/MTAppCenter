...
#import "NSDate+YKExtension.h"

@implementation NSDate (YKExtension)
 
/// 获得格式化的时间字符串
/// @param formateString 时间格式，如 2015-6-3 12:12:12, <formateString==nil>||<formateString==@""> 时使用默认格式 yyyy-MM-dd HH:mm:ss
- (NSString *)yk_stringWithFormate:(NSString *)formateString
{
...
+ (NSDate *)yk_dateWithFormatedDateString:(NSString *)formatedDateString formate:(NSString *)formate
{
...
+ (NSString *)currentDateString {
    return [self yk_currentDateStringWithFormat:@"yyyy-MM-dd HH:mm:ss"];
}
/// 获取当前时间字符串 默认格式 <@"yyyy-MM-dd">
+ (NSString *)currentDateString1{
    return [self yk_currentDateStringWithFormat:@"yyyy-MM-dd"];
}
...
+ (NSString *)yk_currentDateStringWithFormat:(NSString *)formatterStr {
    // 获取系统当前时间
...
- (NSString *)yk_stringWithFormatDate {
    if (!self) {
        return @"";
    }
...
- (NSString *)yk_stringByFormate:(NSString *)format
{
...
- (NSInteger)yk_timestampWithCompareDate:(NSDate *)anotherDate
{
    if (!anotherDate) {
        return -1;
    }
...
/// 当前日期
+(NSString *)yk_currentDateString{
    return [self yk_currentDataYearAgo:0 MonthAgo:0 DayAgo:0];
}


/// 当前年份
- (NSInteger)yk_currentYear
{
    NSDate *currentDate = [NSDate date];
    NSDateComponents *dateComponpents = [self yk_getDateComponentsByDate:currentDate];
    return [dateComponpents year];
}


/// 当前月份
- (NSInteger)yk_currentMonth
{
    NSDate *currentDate = [NSDate date];
    NSDateComponents *dateComponpents = [self yk_getDateComponentsByDate:currentDate];
...
- (NSDateComponents *)yk_getDateComponentsByDate:(NSDate *)date
{
...
+(NSString*)yk_currentDataYearAgo:(NSInteger)yearNum MonthAgo:(NSInteger)monthNum DayAgo:(NSInteger)dayNum{
...
- (NSDate *)yk_dateWithYearOffset:(NSInteger)yearOffset monthOffset:(NSInteger)monthOffset dayOffset:(NSInteger)dayOffset {
    NSCalendar *calendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    //    NSDateComponents *comps = nil;
...
+ (BOOL)yk_isBetweenFromHour:(NSInteger)fromHour toHour:(NSInteger)toHour formin:(NSInteger)min tomin:(NSInteger)tomin
{
    NSDate *dateform = [NSDate yk_getCustomDateWithHour:fromHour min:min sec:0];
    NSDate *dateto = [NSDate yk_getCustomDateWithHour:toHour min:tomin sec:0];
    NSDate *currentDate = [NSDate date];
...
/// @param fromHour 起点的时间-小时
/// @param toHour 终点的时间-小时
+ (BOOL)yk_isBetweenFromHour:(NSInteger)fromHour toHour:(NSInteger)toHour
{
    NSDate *date8 = [NSDate yk_getCustomDateWithHour:fromHour min:0 sec:0];
    NSDate *date23 = [NSDate yk_getCustomDateWithHour:toHour min:59 sec:59];
    NSDate *currentDate = [NSDate date];
...
/// @param min 指定的分钟
/// @param sec 指定的秒
+ (NSDate *)yk_getCustomDateWithHour:(NSInteger)hour min:(NSInteger)min sec:(NSInteger)sec
{
    //获取当前时间
    NSDate *currentDate = [NSDate date];
...
/// 获取下个月的天数
+ (NSInteger)yk_numberOfDaysInNextMonth{
    NSDate *currentDate = [NSDate date];
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
...
- (NSString *)yk_format:(NSString *)formatString{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    formatter.dateFormat = formatString;
    return [formatter stringFromDate:self];
}
...
+ (NSDate *(^)(NSString *,NSString *))yk_format {
